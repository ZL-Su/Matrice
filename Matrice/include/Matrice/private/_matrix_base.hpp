...
public:
	...
	template<typename _Lhs, typename _Rhs, typename _Op>
	MATRICE_GLOBAL_INL Base_(const exprs::Expr::MatBinaryExpr<_Lhs, _Rhs, _Op>& expr) 
		:base_t(m_rows = expr.rows(), m_cols = expr.cols()), m_storage(m_rows, m_cols) { m_data = m_storage.data(); expr.assign(*this); }
	template<typename _Rhs, typename _Op>
	MATRICE_GLOBAL_INL Base_(const exprs::Expr::MatUnaryExpr<_Rhs, _Op>& expr) 
		:base_t(m_rows = expr.rows(), m_cols = expr.cols()), m_storage(m_rows, m_cols) { m_data = m_storage.data(); expr.assign(*this); }

	///<brief> opencv interface </brief>
#ifdef __use_ocv_as_view__
	MATRICE_HOST_FINL Base_(const ocv_view_t& mat) : base_t(mat.rows, mat.cols, mat.data) {}
...
#pragma region <!-- Lazied Operators for Matrix Arithmetic -->
	template<typename _Rhs> MATRICE_GLOBAL_INL exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseSumOp> operator+ (const _Rhs& _opd) const { return exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseSumOp>(*this, _opd);}
	template<typename _Rhs> MATRICE_GLOBAL_INL exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseMinOp> operator- (const _Rhs& _opd) const { return exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseMinOp>(*this, _opd); }
	template<typename _Rhs> MATRICE_GLOBAL_INL exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseMulOp> operator* (const _Rhs& _opd) const { return exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseMulOp>(*this, _opd); }
	template<typename _Rhs> MATRICE_GLOBAL_INL exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseDivOp> operator/ (const _Rhs& _opd) const { return exprs::Expr::EwiseBinaryExpr<_Myt, _Rhs, EwiseDivOp>(*this, _opd); }
	template<typename _Rhs> MATRICE_GLOBAL_INL exprs::Expr::MatBinaryExpr<_Myt, _Rhs, MatMulOp> mul(const _Rhs& rhs) { return exprs::Expr::MatBinaryExpr<_Myt, _Rhs, MatMulOp>(*this, rhs); }
	MATRICE_HOST_FINL exprs::Expr::MatUnaryExpr<_Myt, MatInvOp> inv() { return exprs::Expr::MatUnaryExpr<_Myt, MatInvOp>(*this); }
	MATRICE_HOST_FINL exprs::Expr::MatUnaryExpr<_Myt, MatInvOp> inv(const_myt_ref _rhs) { return exprs::Expr::MatUnaryExpr<_Myt, MatInvOp>(_rhs, *this); }
	MATRICE_HOST_FINL exprs::Expr::MatUnaryExpr<_Myt, MatTrpOp> transpose() { return exprs::Expr::MatUnaryExpr<_Myt, MatTrpOp>(*this); }
	MATRICE_GLOBAL_INL exprs::Expr::EwiseBinaryExpr<_Myt, _Myt, EwiseMulOp> normalize(value_t _val = inf) { return ((std::abs(_val) < eps ? value_t(1) : value_t(1) / (_val == inf ? max() : _val))*(*this)); }
	MATRICE_GLOBAL_INL exprs::Expr::MatBinaryExpr<_Myt, _Myt, MatMulOp> spread();
#pragma endregion

#pragma region <!-- Triggers for Suspended Expression -->
	template<typename _Lhs, typename _Rhs, typename _Op>
	MATRICE_GLOBAL_INL _Derived& operator= (const exprs::Expr::EwiseBinaryExpr<_Lhs, _Rhs, _Op>& expr) { expr.assign(*this); return (*static_cast<_Derived*>(this)); }
	template<typename _Lhs, typename _Rhs, typename _Op>
	MATRICE_GLOBAL_INL _Derived& operator= (const exprs::Expr::MatBinaryExpr<_Lhs, _Rhs, _Op>& expr) { expr.assign(*this); return (*static_cast<_Derived*>(this)); }
	template<typename _Lhs, typename _Rhs, typename _Op>
	MATRICE_GLOBAL_INL _Derived& operator= (const exprs::Expr::MatUnaryExpr<_Myt, _Op>& expr) { expr.assign(*this); return (*static_cast<_Derived*>(this)); }
#pragma endregion
	...
	constexpr MATRICE_HOST_INL format_t _Prop_format_getter() const { return m_format; }
	__declspec(property(get = _Prop_empty_getter)) bool empty;
	constexpr MATRICE_HOST_INL bool _Prop_empty_getter() const { return (size() == 0); }
	constexpr MATRICE_GLOBAL_INL type_t& storage_type() { return m_type; }
	constexpr MATRICE_GLOBAL_INL type_t storage_type() const { return m_type; }
