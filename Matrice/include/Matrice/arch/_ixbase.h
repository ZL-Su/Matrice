...
{
	using _Myop_type = detail::Op_<T>;
	typename _Myop_type::template adaptor<_Elems> _Myop;
public:
	enum { size = _Elems };
	using op_t           = _Myop_type;
	using value_t        = T;
	...
	MATRICE_HOST_FINL auto& operator= (initlist_t _arg) { 
		m_data = _Myop(pointer(_arg.begin())); 
		return(*this); 
	}
	MATRICE_HOST_FINL auto& operator= (const_internal _arg) {
		m_data = _arg; 
		return(*this); 
	}
	MATRICE_HOST_FINL constexpr auto& operator[](size_t i) { 
		return _Myop(m_data)[i]; 
	}
	MATRICE_HOST_FINL constexpr const auto& operator[](size_t i) const { 
		return _Myop(m_data)[i]; 
	}
	MATRICE_HOST_FINL auto operator()(pointer data) const { 
		_Myop(m_data, data); 
	}
	template<typename _Fwdty, MATRICE_ENABLE_IF(has_data_v<_Fwdty>)>
	MATRICE_HOST_FINL auto operator()(_Fwdty& arg) const { 
		_Myop(m_data, arg.data()); 
	}
	...
	MATRICE_HOST_FINL auto reduce()noexcept { 
		return (_Myop + begin()); 
	}
	MATRICE_HOST_FINL const auto reduce()const noexcept { 
		return (_Myop + begin()); 
	}

	// \unpack to memory block that 'data' point to 
	MATRICE_HOST_FINL auto unpack(pointer data) const noexcept {
		_Myop(m_data, data); 
	}
	// \unpack to a container 'arg' with method data()
	template<typename _Fwdty, MATRICE_ENABLE_IF(has_data_v<_Fwdty>)>
	MATRICE_HOST_FINL auto unpack(_Fwdty& arg) const noexcept {
		_Myop(m_data, arg.data()); 
	}

	// \evaluate the vectorizable size for given 'length'
	MATRICE_HOST_FINL auto vsize(size_t _Len) const noexcept { 
