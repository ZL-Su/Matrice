...
class span {
    static_assert (std::is_array_v<T> == false, "can't have span of an array");
public:
    ...
    template<typename _Ity, MATRICE_ENABLE_IF(_Ity)>
    ...
    constexpr span<element_type, Count> subspan () const {
        return { m_data + Offset, 
            ...
            (Extent != dynamic_extent ? Extent - Offset : m_size - Offset) };
    }
    ...
    reference at (size_type idx) const {
        if (idx >= size())
            throw (std::out_of_range ("dgelom::span::at"));
        return m_data[idx];
    }
...
/// cspan<T> is an alias for a non-mutable span<const T>.
template <typename T>
...
public:
    ...
    // assignments
    span_strided& operator= (const span_strided &other) {
        m_data   = other.data();
        m_size   = other.size();
    ...
    reference at (size_type idx) const {
        if (idx >= size())
            throw (std::out_of_range ("dgelom::span_strided::at"));
        return m_data[m_stride*idx];
    }
    constexpr reference front() const noexcept { return m_data[0]; }
...
/// cspan_strided<T> is an alias for a non-mutable span_strided<const T>.
template <typename T>
...
template<class T, size_t E = dgelom::dynamic_extent>
constexpr size_t size(const dgelom::span<T, E>& c) {
    return static_cast<size_t>(c.size());
}

template<class T, size_t E = dgelom::dynamic_extent>
constexpr size_t size(const dgelom::span_strided<T, E>& c) {
    return static_cast<size_t>(c.size());
}

#ifndef MATRICE_ENABLE_CXX20
// C++20 and beyond already have these declared.
template<class T, size_t E = dgelom::dynamic_extent>
constexpr ptrdiff_t ssize(const dgelom::span<T, E>& c) {
    return static_cast<ptrdiff_t>(c.size());
}

template<class T, size_t E = dgelom::dynamic_extent>
constexpr ptrdiff_t ssize(const dgelom::span_strided<T, E>& c) {
    return static_cast<ptrdiff_t>(c.size());
}
#endif
...
// our span templates.
#define MATRICE_SPAN_HAS_STD_SIZE 1
